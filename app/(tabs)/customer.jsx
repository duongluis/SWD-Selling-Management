...
export default function CustomerView() {
  ...
  // Tự động refetch mỗi khi màn hình được focus (ví dụ: quay lại từ addCustomer)
  useFocusEffect(
    useCallback(() => {
      refresh();
    }, [refresh])
  ...
  // ── Gom nhóm customers theo createdBy ────────────────────
    ...
    if (role === "daily" || role === "phantan") {
      ...
      customers.forEach(c => {
        if (c.createdBy && c.createdBy !== myEmail) {
          if (!subMap.has(c.createdBy)) {
            subMap.set(c.createdBy, { email: c.createdBy, name: c.createdBy, customers: [] });
          }
